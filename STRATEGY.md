...
| **C · Code-mode (laddered)**            | Agents author governed scripts composing `@macts/*` in one execution; **run/approve/reuse via their own harness** (macts governs the ops, Epic B). Rung 1 = authoring + steering (now); Rung 2 = macts-side run/MCP surfaces; Rung 3 = hardened sandbox (deferred, ~separate product). Two owners: the harness gates _running_ code; macts governs what it does to apps. | §5.1, §9, §14        | **Greenlit (R1)**       |
