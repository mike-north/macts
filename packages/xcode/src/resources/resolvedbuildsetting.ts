...
import type {
  ...
  ResolvedBuildSettingCreateInput,
  ResolvedBuildSettingUpdateInput,
} from '../types.js'
...
export class ResolvedBuildSettingResourceClient {
  ...
  async update(
    name: string,
    input: ResolvedBuildSettingUpdateInput
  ): Promise<ResolvedBuildSetting> {
    return this.#http.rpc<ResolvedBuildSetting>(`${this.#app}.${this.#resource}.update`, {
      name,
      ...input,
    })
  }
