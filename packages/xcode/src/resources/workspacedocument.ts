...
import type { WorkspaceDocument, WorkspaceDocumentCreateInput, WorkspaceDocumentUpdateInput } from '../types.js';
...
export class WorkspaceDocumentResourceClient {
  ...
  async update(name: string, input: WorkspaceDocumentUpdateInput): Promise<WorkspaceDocument> {
    return this.#http.rpc<WorkspaceDocument>(`${this.#app}.${this.#resource}.update`, { name, ...input });
  }
