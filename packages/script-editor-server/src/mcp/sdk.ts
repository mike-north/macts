...
import { ScriptEditorClient, type ScriptEditorClientOptions } from '@macts/script-editor'
  ...
  if (!apiKey) {
    throw new Error(
      'MACTS_API_KEY environment variable is required. ' +
        'Create an API key with: macts api-key create --name "<name>" --permission script-editor:*:*'
    )
  }
