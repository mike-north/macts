...
export class CreateSoundAlarmCommand extends Command {
  ...
  triggerInterval = Option.String('--trigger-interval', { required: true, description: "The interval in minutes between the event and the alarm (positive for after, negative for before)" });
  triggerDate = Option.String('--trigger-date', { required: true, description: "An absolute alarm date" });
  soundName = Option.String('--sound-name', { required: true, description: "The system sound name to be used for the alarm" });
  soundFile = Option.String('--sound-file', { required: true, description: "The (POSIX) path to the sound file to be used for the alarm" });
