...
export class CreateSocialProfileCommand extends Command {
  static override paths = [['contacts', 'people', 'socialProfiles', 'create']]
  ...
  serviceName = Option.String('--service-name', {
    required: true,
  ...
  userIdentifier = Option.String('--user-identifier', {
    required: true,
  ...
  async execute(): Promise<number> {
    ...
    try {
      ...
      const item = await client.socialprofiles.create({
        ...
        url: this.url,
      } as unknown as Parameters<typeof client.socialprofiles.create>[0])
