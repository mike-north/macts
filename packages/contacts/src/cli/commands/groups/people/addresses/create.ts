...
export class CreateAddressCommand extends Command {
  static override paths = [['contacts', 'groups', 'people', 'addresses', 'create']]
  ...
  async execute(): Promise<number> {
    ...
    try {
      ...
      const item = await client.addresses.create({
        ...
        state: this.state,
      } as unknown as Parameters<typeof client.addresses.create>[0])
