...
import type { RequiredResource, RequiredResourceCreateInput, RequiredResourceUpdateInput } from '../types.js';
...
export class RequiredResourceResourceClient {
  ...
  async update(name: string, input: RequiredResourceUpdateInput): Promise<RequiredResource> {
    return this.#http.rpc<RequiredResource>(`${this.#app}.${this.#resource}.update`, { name, ...input });
  }
