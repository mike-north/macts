...
describe('paths', () => {
  ...
  describe('getMactsHome', () => {
    ...
    it('should fall back to ~/.macts when MACTS_HOME is empty string', () => {
      ...
      // returned as-is, making manifest/log paths cwd-relative.
  ...
  describe('getPluginsPackageJson', () => {
    it('should return correct path', () => {
      delete process.env['MACTS_HOME']
      const path = getPluginsPackageJson()
      expect(path).toBe(join(homedir(), '.macts', 'plugins', 'package.json'))
    })

    it('should respect MACTS_HOME override', () => {
      process.env['MACTS_HOME'] = '/custom/path'
      const path = getPluginsPackageJson()
      expect(path).toBe('/custom/path/plugins/package.json')
    })
  })
  ...
  describe('getPluginsCacheFile', () => {
    it('should return correct path', () => {
      delete process.env['MACTS_HOME']
      const path = getPluginsCacheFile()
      expect(path).toBe(join(homedir(), '.macts', 'plugins', '.plugins-cache.json'))
    })

    it('should respect MACTS_HOME override', () => {
      process.env['MACTS_HOME'] = '/custom/path'
      const path = getPluginsCacheFile()
      expect(path).toBe('/custom/path/plugins/.plugins-cache.json')
    })
  })
