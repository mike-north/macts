...
describe('registerAllPlugins', () => {
  ...
  it('should include load errors in result', () => {
    const loadErrors: PluginLoadError[] = [
      { packageName: '@macts/broken', reason: 'load-error', message: 'Module not found' },
      { packageName: '@macts/invalid', reason: 'load-error', message: 'Invalid plugin export' },
    ]
    const discoveryResult: PluginDiscoveryResult = {
      plugins: [],
      errors: loadErrors,
  ...
  it('should handle discovery result with only errors', () => {
    const loadErrors: PluginLoadError[] = [
      { packageName: '@macts/failed', reason: 'load-error', message: 'Failed to load' },
    ]
    const discoveryResult: PluginDiscoveryResult = {
      plugins: [],
      errors: loadErrors,
  ...
  it('should handle mixed plugins and errors', () => {
    ...
    const loadErrors: PluginLoadError[] = [
      { packageName: '@macts/broken', reason: 'load-error', message: 'Load failed' },
    ]
    const discoveryResult: PluginDiscoveryResult = {
      plugins: [plugin],
      errors: loadErrors,
