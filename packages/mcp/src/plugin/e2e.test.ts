  ...
  describe('discoverMcpPlugins', () => {
    ...
    it('should discover a valid MCP plugin from @macts/test-app-server/mcp', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'test-app-server',
        validMcpPluginSource('test-app', 'Test app MCP plugin', tools)
      )
      await pluginsProject.write()
    ...
    it('should discover multiple server plugins', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'alpha-server',
        validMcpPluginSource('alpha', 'Alpha plugin', toolsA)
      )
      addMcpPlugin(
        pluginsProject,
        'beta-server',
        validMcpPluginSource('beta', 'Beta plugin', toolsB)
      )
      await pluginsProject.write()
    ...
    it('should return error when plugin has invalid tool definitions (missing handler)', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'invalid-tools-server',
        validMcpPluginSource('invalid-tools', 'Invalid tools plugin', tools)
      )
      await pluginsProject.write()
    ...
    it('should return error when plugin has invalid tool definitions (null inputSchema)', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'null-schema-server',
        validMcpPluginSource('null-schema', 'Null schema plugin', tools)
      )
      await pluginsProject.write()
    ...
    it('should validate tool structure (name, description, inputSchema, handler)', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'valid-tools-server',
        validMcpPluginSource('valid-tools', 'Plugin with valid tools', `[${tool}]`)
      )
      await pluginsProject.write()
    ...
    it('should return both plugins and errors when mix of valid and broken', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'good-app-server',
        validMcpPluginSource('good-app', 'Good MCP plugin', tools)
      )
  ...
  describe('cache integration', () => {
    it('should use cache on second discovery when lockfile exists', async () => {
      ...
      addMcpPlugin(
        pluginsProject,
        'cached-app-server',
        validMcpPluginSource('cached-app', 'Cached MCP plugin', tools)
      )
      await pluginsProject.write()
