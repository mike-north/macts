/**
 * Plugin cache for fast MCP server startup.
 *
...
import { getPluginsLockfile, getMcpPluginsCacheFile } from './paths.js'
...
/**
 * Read the MCP plugin cache if valid.
 *
 * Returns null if:
...
export function readMcpPluginCache(): readonly CachedPlugin[] | null {
  const cacheFile = getMcpPluginsCacheFile()
...
/**
 * Invalidate the MCP plugin cache.
 *
