...
import type { OutgoingMessage, OutgoingMessageCreateInput, OutgoingMessageUpdateInput } from '../types.js';
...
export class OutgoingMessageResourceClient {
  ...
  async update(id: string, input: OutgoingMessageUpdateInput): Promise<OutgoingMessage> {
    return this.#http.rpc<OutgoingMessage>(`${this.#app}.${this.#resource}.update`, { id, ...input });
  }
