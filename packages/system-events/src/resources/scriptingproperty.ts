...
import type { ScriptingProperty, ScriptingPropertyCreateInput, ScriptingPropertyUpdateInput } from '../types.js';
...
export class ScriptingPropertyResourceClient {
  ...
  async update(id: string, input: ScriptingPropertyUpdateInput): Promise<ScriptingProperty> {
    return this.#http.rpc<ScriptingProperty>(`${this.#app}.${this.#resource}.update`, { id, ...input });
  }
