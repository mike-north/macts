...
export class CreateSharedTrackCommand extends Command {
  static override paths = [['tv', 'sources', 'libraryPlaylists', 'sharedTracks', 'create']]
  ...
  async execute(): Promise<number> {
    ...
    try {
      ...
      const item = await client.sharedtracks.create(
        {} as unknown as Parameters<typeof client.sharedtracks.create>[0]
      )
