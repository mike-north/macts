...
function generateIndexFile(ctx: GeneratorContext, appClassName: string): string {
  ...
  for (const resource of ctx.getResources()) {
    lines.push(`export type { ${resource.name}, ${resource.name}CreateInput, ${resource.name}UpdateInput } from './types/${resource.name}.js';`);
  }
  ...
  for (const resource of ctx.getResources()) {
    lines.push(`export { ${resource.name}Schema, ${resource.name}CreateInputSchema, ${resource.name}UpdateInputSchema } from './schemas/${resource.name}Schema.js';`);
  }
