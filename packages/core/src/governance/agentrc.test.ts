/**
 * Runtime tests for the `.agentrc` / org-policy declaration parser.
 *
...
describe('parseAgentRc — valid declarations', () => {
  it('accepts a fully-specified declaration and preserves every field', () => {
    const result = parseAgentRc(baseDeclaration())
    expect(result.success).toBe(true)
    if (!result.success) return // narrow for TS

    const data: AgentRc = result.data
  ...
  it.each(AGENT_RC_DISPOSITIONS)('accepts the "%s" disposition', (disposition) => {
    const result = parseAgentRc({ apps: [{ app: 'calendar', disposition }] })
    expect(result.success).toBe(true)
    if (!result.success) return
    expect(result.data.apps[0]?.disposition).toBe<AgentRcDisposition>(disposition)
  })
  ...
  it('exposes the canonical disposition set in least-to-most-restrictive order', () => {
    ...
    expect(AGENT_RC_DISPOSITIONS).toEqual(['allowed', 'read-only', 'confirm-first', 'forbidden'])
  })
})

describe('parseAgentRc — invalid declarations (negative, one constraint each)', () => {
  it('rejects a non-object input', () => {
    const result = parseAgentRc('not an object')
  ...
  it('rejects null', () => {
    const result = parseAgentRc(null)
    expect(result.success).toBe(false)
  })

  it('rejects an unknown top-level key (strict schema)', () => {
    const result = parseAgentRc({ apps: [], unexpected: true })
    expect(result.success).toBe(false)
    if (result.success) return
  ...
  it('rejects a wrong version literal', () => {
    const result = parseAgentRc({ version: '2', apps: [] })
    expect(result.success).toBe(false)
    if (result.success) return
  ...
  it('rejects an unknown disposition value', () => {
    const result = parseAgentRc({ apps: [{ app: 'calendar', disposition: 'maybe' }] })
    expect(result.success).toBe(false)
    if (result.success) return
  ...
  it('rejects a missing required app disposition', () => {
    const result = parseAgentRc({ apps: [{ app: 'calendar' }] })
    expect(result.success).toBe(false)
    if (result.success) return
    expect(result.issues.some((i) => i.path === 'apps.0.disposition')).toBe(true)
  })
  ...
  it('rejects an uppercase app name (must be lowercase identifier or "*")', () => {
    const result = parseAgentRc({ apps: [{ app: 'Calendar', disposition: 'allowed' }] })
    expect(result.success).toBe(false)
    if (result.success) return
  ...
  it('rejects an empty app name', () => {
    const result = parseAgentRc({ apps: [{ app: '', disposition: 'allowed' }] })
    expect(result.success).toBe(false)
    if (result.success) return
    expect(result.issues.some((i) => i.path === 'apps.0.app')).toBe(true)
  })
  ...
  it('rejects an empty sensitivity tag', () => {
    const result = parseAgentRc({ tags: [''] })
    expect(result.success).toBe(false)
    if (result.success) return
  ...
  it('rejects a non-array apps field', () => {
    const result = parseAgentRc({ apps: 'calendar' })
    expect(result.success).toBe(false)
    if (result.success) return
  ...
  it('rejects an empty reason string (must be non-empty when present)', () => {
    const result = parseAgentRc({ apps: [{ app: 'calendar', disposition: 'allowed', reason: '' }] })
    expect(result.success).toBe(false)
    if (result.success) return
