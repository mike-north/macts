...
export interface CallAuditContext {
  ...
  /**
   * Correlation id for the approval request this call would raise, generated by
   * the caller *before* enforcement so that the `pending` record and the later
   * `approved`/`rejected` record can be paired.
   *
   * Without it, two identical overlapping calls from the same key produce
   * indistinguishable records and an audit consumer cannot tell which
   * resolution belongs to which hold.
   *
   * {@link enforceCall} records it **only** on a `pending` decision — an
   * `allowed` or `denied` call never raises an approval request, so carrying the
   * id there would be noise. {@link recordApprovalDecision} always records it.
   */
...
export async function enforceCall(options: EnforceCallOptions): Promise<EnforcementDecision> {
  ...
  if (writer !== undefined) {
    ...
    const record = createAuditRecord({
      ...
      // Only a withheld call goes on to raise an approval request, so only its
      // record needs the correlation id.
      ...(outcome === 'pending-approval' && audit.approvalId !== undefined
...
export interface RecordApprovalDecisionOptions {
  ...
  /**
   * Audit context for this call (see {@link CallAuditContext}). Its
   * `approvalId` should be the same one carried on the `pending` record.
   */
  readonly audit: CallAuditContext
  /**
   * Audit writer. **Required**, unlike {@link EnforceCallOptions.writer}: a
   * human decision that resolves a held call must always be persisted, so there
   * is no "decide but do not record" mode to fall into.
   */
 ...
 * record's human-readable reason.
 *
 * The record carries {@link ApprovalOutcome.auditReason}, not
 * {@link ApprovalOutcome.reason}: the audit trail is the place for the full
 * detail (including third-party exception text), while the client-safe reason
 * is what a caller sees.
 *
 * The write is not optional. If it fails, this rejects — and the caller must
 * treat that as a denial, because an approved call it cannot record is a call
 * that must not run.
 *
 * @param options - Permission, approval outcome, audit context, and writer.
