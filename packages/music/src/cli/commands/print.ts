...
export class PrintCommand extends Command {
  ...
  printDialog = Option.Boolean('--print-dialog', { description: "Should the application show the print dialog" });
  withProperties = Option.String('--with-properties', { required: false, description: "the print settings" });
  kind = Option.String('--kind', { required: false, description: "the kind of printout desired" });
  theme = Option.String('--theme', { required: false, description: "name of theme to use for formatting the printout" });
